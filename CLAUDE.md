...
Zero external dependencies — Swift + Apple frameworks (Foundation, CoreServices, Carbon) plus a self-contained C index library (`Sources/CIndex/`, built via SPM + standalone Makefile). The hot index path is pure C (single-allocation `DFileMeta` entries, sorted-name prefix array, trigram inverted index) wrapped by a thin Swift actor (`InMemoryIndex`). Persistence is a custom binary format (`index.bin` + `index.cursor` sidecar); SQLite3 is retained **only** for the one-time legacy-DB→binary migration (`LegacySQLiteReader`). CLI via Darwin.readline + ANSI escape codes.
...
Sources/
  ...
  Persist/                    # IndexPersistence (binary index.bin actor), BinaryIndex (DFIX engine + cursor sidecar), LegacySQLiteReader (one-time db→bin migration), IndexRecovery, PathEncryption, SchemaMigrator (SQLite, migration-only), SecretsStore
