...
2. 输入 API Key（`deepfinder config set ai.api_key <key>`，存储在本地 ~/.deep-finder/.env（permissions 600），不上传）
3. 开启 "发送文件元数据"（`deepfinder config set ai.send_metadata true`，默认关闭）
4. 回到 CLI/GUI，用自然语言搜索

**验收标准**：
- [ ] DeepSeekProvider 实现 AIModelProvider
- [ ] API Key 存储在 ~/.deep-finder/.env（permissions 600，不存明文）
...
- [ ] API Key 存储在 ~/.deep-finder/.env（permissions 600）
...
- [ ] [配置] API Key（`ai.api_key`，存储到 ~/.deep-finder/.env（permissions 600），不存明文）
