...
- [ ] PID 文件管理：`~/.deep-finder/session/daemon.pid`，启动时写入 PID，退出时清理
...
**验收标准**：
- [ ] Unix domain socket 服务器：`~/.deep-finder/session/ipc.sock`
...
- [ ] 可通过 `nc -U ~/.deep-finder/session/ipc.sock` 手动调试（发送 JSON 请求）
...
**验收标准**：
- [ ] LaunchAgent 集成：`~/Library/LaunchAgents/cn.com.nadav.deepfinder.daemon.plist`（v0.7 `deepfinder install` 安装）
