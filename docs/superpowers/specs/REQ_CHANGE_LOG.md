...
> 备注（待后续单独处理）：REQ_STATUS.md 中 REQ-3.0-03/04 的 Notes 仍写「Keychain API key」，与 CHG-2026-06-03-01（确认使用 `.env` 600，不迁移 Keychain）及 REQ 卡片原文冲突——属既有内容错误，不在本次格式重构范围内。
