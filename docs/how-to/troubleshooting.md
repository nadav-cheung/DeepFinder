...
3. **Index is stale** -- The index can get out of sync after unexpected daemon shutdowns or filesystem errors. Rebuild it with `deepfinder daemon rebuild`. This forces a full re-scan.

If this didn't help, run `deepfinder diagnose` and [open an issue](../SUPPORT.md).
...
2. **launchd conflict** -- A stale LaunchAgent registration may be holding a reference. Check with `launchctl list | grep deepfinder`. If a stale entry exists, unload it: `launchctl unload ~/Library/LaunchAgents/com.nadav.deepfinder.plist`, then restart.
...
2. **External volume mounted** -- External drives (especially network volumes or slow USB disks) add latency. Check active volumes with `deepfinder daemon volumes`. Unmount unnecessary volumes or add slow paths to `excludedPaths`.
3. **AI search timeout** -- If you are using AI semantic search (v3.0+), network latency to the AI provider can slow things down. Verify connectivity (`curl https://api.deepseek.com` or your provider's endpoint). Increase the timeout with `deepfinder config set aiTimeout 30` (seconds).
...
2. **Conflict with another app** -- Another application may have registered the same shortcut. Try a different hotkey: `deepfinder config set hotkey "cmd+shift+space"`. Restart the GUI app after changing.
...
If this didn't help, use the menu bar icon to open the search panel, then run `deepfinder diagnose` and [open an issue](../SUPPORT.md).

## AI Features Not Working

Semantic search, image similarity, or other AI-powered features return errors or silently fall back.

1. **API key not configured** -- Each AI provider needs its API key set. Check with `deepfinder config get` or set it directly: `deepfinder config set deepseekApiKey "sk-..."` (or `anthropicApiKey`, `openaiApiKey`, `geminiApiKey` depending on your chosen provider).
...
2. **Code signature verification failed** -- macOS Gatekeeper may block an unsigned or tampered binary. Download from the official [GitHub Releases](https://github.com/nadav/DeepFinder/releases) page and verify the SHA-256 checksum listed there. Do not bypass Gatekeeper unless you have verified the checksum.
...
If this didn't help, download the DMG directly from [GitHub Releases](https://github.com/nadav/DeepFinder/releases), or run `deepfinder diagnose` and [open an issue](../SUPPORT.md).
